...
const getOneBusinessById = async (id) => {
   const user = await db.Business.findOne({ where: { id }, attributes: { exclude: ['otp_code'] } });
   return user;
};

const handleGetAllBusinesses = async () => {
   try {
      ...
      businesses.forEach(business => {
         if (business.otp_code.length < 1) {
            dataFilterConfirmed.push(business);
         }
      });

      return handleSuccess("All Business retrieved successfully", dataFilterConfirmed);
   } catch (error) {
      throw error;
   }
};
...
const handleGetAllBusinessesApproved = async () => {
   try {
      const Businesss = await db.Business.findAll({
         where: { approved: true }
      })
      return handleSuccess("All Business retrieved successfully", Businesss);
   } catch (error) {
      throw error
   }
}

const handleGetOneBusiness = async (id) => {
   try {
      const Business = await getOneBusinessById(id)

      if (!Business) {
         throw createError.NotFound("Business not found")
      }

      return handleSuccess("Business retrieved successfully", Business);
   } catch (error) {
      throw error
   }
}

const handleCreateBusiness = async (name_company, type_company, establishment_date, business_registration_number, address, career, number_of_employees, certification_image, front_cccd_image, back_cccd_image, email, address_wallet) => {
   try {
      // Tạo mã xác minh 
      ...
      const newBusiness = await db.Business.create({
         name_company, type_company, establishment_date, business_registration_number, address, career, number_of_employees, certification_image, front_cccd_image, back_cccd_image, otp_code, email, address_wallet
      });

      const result = newBusiness.toJSON();
      delete result.otp_code;

      await sendMail({ email, subject: "XAC THUC FORM THONG TIN CUA DOANH NGHIEP", html })
...
const handleConfirmCreateBussiness = async (otp_code) => {
   try {
      const result = await db.Business.findOne({ where: { otp_code } })
      if (!result) {
      ...
      if (timeDifference > 5) {
         await db.Business.destroy({ where: { id: result.id } })
         throw createError.BadRequest('OTP expired (more than 5 minutes). Please fill out your form again!');
      }

      await db.Business.update({ otp_code: "" }, { where: { id: result.id } })
...
const handleApproveBusiness = async (id) => {
   try {
      ...
      const result = await db.Business.findOne({ where: { id } })
      if (!result) {
         throw createError.NotFound('Form business not found');
      }
      await db.Business.update({ approved: true }, { where: { id: result.id } })
...
const handleDeleteBusiness = async (id) => {
   try {
      ...
      return handleSuccess("Business deleted successfully");
   } catch (error) {
      throw error;
   }
};
...
module.exports = {
   ...
   handleGetAllBusinessesApproved,
   handleGetOneBusiness,
   handleCreateBusiness,
   handleDeleteBusiness,
   handleConfirmCreateBussiness,
